...
public class UsuariosController : Controller
{
    ...
    public async Task<IActionResult> Create(CrearUsuarioViewModel model)
    {
        ...
        try
        {
            ...
            if (registrado)
            {
                ...
                return Json(new { success = true, mensaje = "Usuario creado exitosamente", redirectUrl = Url.Action("Index") });
            }
    ...
    public async Task<IActionResult> CambiarEstado(int idUsuario, bool estado)
    {
        try
        {
            ...
            return Json(new { success = filas > 0, mensaje = filas > 0 ? "Estado actualizado" : "Error al actualizar estado" });
        }
        catch (Exception ex)
        {
